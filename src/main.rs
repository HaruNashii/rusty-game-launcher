...
fn main() 
{
    ...
    let fonts_objects = create_layout(vec![config_file_data.text_position[0], config_file_data.text_position[1], files.len() as i32, config_file_data.object_per_line, config_file_data.distance_between_texts[0], config_file_data.distance_between_texts[1]], config_file_data.gride_type);
    let images_objects = create_layout(vec![config_file_data.image_position[0], config_file_data.image_position[1], files.len() as i32, config_file_data.object_per_line, config_file_data.distance_between_images[0], config_file_data.distance_between_images[1]], config_file_data.gride_type);
    let (texture_creator, mut canvas, mut event_pump) = create_window(&config_file_data.window_size); 
    let gride_data = (config_file_data.gride_type, &fonts_objects);
    ...
    loop 
    {
        ...
        let (clicked, option_returned) = handle_input(&config_file_data.image_position, &config_file_data.window_size, gride_data, selected_option, config_file_data.object_per_line, fonts.ui_rect_vector.len(), &mut event_pump);
